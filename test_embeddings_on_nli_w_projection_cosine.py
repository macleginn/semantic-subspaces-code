...
def train_epoch(epoch_n, batch_size, data, layer, model, optimiser):
    ...
    for step_n in tqdm(range(n_steps), desc=f'Epoch {epoch_n+1}, training', leave=False):
        ...
        hypotheses = data['hypotheses'][layer][lo:hi]
        ...
        projected_hypotheses = model(hypotheses)
        scores = torch.nn.functional.cosine_similarity(
            projected_premises,
            projected_hypotheses,
            dim=-1
        )
        # Cosine similarities corresponding to labels == 0 should be close to 1.
        # Cosine similarities corresponding to labels == 2 should be close to -1.
        gold_scores = torch.tensor(list(map(
            lambda l: 1.0 if l == 0 else -1.0,
            labels)))
        if model.using_cuda:
            gold_scores = gold_scores.cuda()

        loss = torch.square(scores - gold_scores).mean()
        loss.backward()
        optimiser.step()
...
def validate_epoch(epoch_n, batch_size, data, layer, model):
    ...
    for step_n in tqdm(range(n_steps), desc=f'Epoch {epoch_n+1}, validation', leave=False):
        ...
        for score, label in zip(scores, labels):
            if (label == 0 and score > 0) or (label == 2 and score < 0):
                hits += 1
            else:
                misses += 1
...
def encode_sentences_w_gpt(job_name, sentences, tokeniser, model):
    ...
    for sentence in tqdm(sentences, desc=job_name, leave=False):
        ...
        try:
            ...
            print(f'Error! {e} on {sentence}')
            print('Retrying with a shorter input')
            tokenisation = tokeniser(
                sentence,
                truncation=True,
                max_length=256,
...
def main():
    task = sys.argv[1]  # snli / anli
    model_name = sys.argv[2]
    model_name_path = model_name.replace('/', '_')
    # Encode everything
    data = {
        'train': {'premises': [], 'hypotheses': [], 'labels': []},
        'dev': {'premises': [], 'hypotheses': [], 'labels': []},
        'test': {'premises': [], 'hypotheses': [], 'labels': []}
    }
    for split in ['train', 'dev', 'test']:
        df = pd.read_csv(f'../data/{task}_1D_{split}.tsv', sep='\t').dropna()
        # Testing
        # df = df.iloc[:5000,]
        for row in df.itertuples():
            ...
            if row.Label != 1:
                ...
                data[split]['hypotheses'].append(row.Hypothesis)
                data[split]['labels'].append(row.Label)
        print(f"{len(data[split]['premises'])} {split} instances")
    ...
    if 'bert' in model_name or 'roberta' in model_name or 'electra' in model_name or 't5' in model_name.lower():
        with torch.no_grad():
            for split in data:
                ...
                for category in ['premises', 'hypotheses']:
                ...
                embeddings[split]['labels'] = data[split]['labels']
    # Causal LM models
    else:
        with torch.no_grad():
            for split in data:
    ...
    for experiment_n in range(1, n_experiments+1):
        out_table_path = f'../tables/{task}_1D_{model_name_path}_{experiment_n}.csv'
